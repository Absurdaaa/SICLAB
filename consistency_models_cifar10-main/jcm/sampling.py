...
def get_sampling_fn(config, sde, model, shape, eps=1e-3):
    ...
    elif sampler_name.lower() == "multistep":
        sampling_fn = get_multistep_sampler(
            config=config,
            sde=sde,
            model=model,
            shape=shape,
...
def get_multistep_sampler(config, sde, model, shape):
    num_steps = int(config.sampling.n_steps)
    if num_steps < 2:
        raise ValueError("multistep sampling requires config.sampling.n_steps >= 2")

    s_noise = float(getattr(config.sampling, "s_noise", 1.0))
    t_start = sde.t_max ** (1 / sde.rho)
    t_end = sde.t_min ** (1 / sde.rho)
    sigmas = (
        t_start + jnp.arange(num_steps) / (num_steps - 1) * (t_end - t_start)
    ) ** sde.rho
    ...
    def sampler(rng, state):
        ...
        def loop_body(i, x):
            sigma = sigmas[i]
            sigma_vec = jnp.ones((x.shape[0],)) * sigma
            denoised = model_fn(x, sigma_vec)
            sigma_next = sigmas[i + 1]
            noise_scale = jnp.sqrt(
                jnp.maximum(sigma_next**2 - sde.t_min**2, 0.0)
            )
            noise = jax.random.normal(next(rng), x.shape) * s_noise
            return denoised + noise * noise_scale

        x = jax.lax.fori_loop(0, num_steps - 1, loop_body, x)
        sigma_vec = jnp.ones((x.shape[0],)) * sigmas[-1]
        samples = model_fn(x, sigma_vec)
        return samples, num_steps
